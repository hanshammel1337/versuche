...
class MovingTf(object):
	def __init__(self):
		...
		self._pub_marker = rospy.Publisher("visualization_marker", Marker, latch = True, queue_size=10) # latch makes sure that the last published message reaches its target
	...
	def get_parameters(self):
		...
		if self.movement_type == "line":
			x_s = rospy.get_param("~x_start", 0)
			y_s = rospy.get_param("~y_start", 0)
			z_s = rospy.get_param("~z_start", 0)
			self._tf_start_coordinates = [x_s, y_s, z_s]

			x_d = rospy.get_param("~x_direction", 0.1) 
			y_d = rospy.get_param("~y_direction", 0)
			z_d = rospy.get_param("~z_direction", 0)
	...
	def move_tf_linear(self):
		self._tf_coordinates = map(add, self._tf_coordinates, self._movement_vector)
		#print self._tf_coordinates
	...
	def move_tf_points(self):
		if self.zero%2==0 and self.time2 + 5 < time.time():
			self._tf_coordinates=[self._tf_coordinates[0]-0.3, self._tf_coordinates[1]-0.1, self._tf_coordinates[2]]
			#time.sleep(15)
			self.time2 = time.time()
	...
	def move_tf_circle(self, time):
		#circle function
		circle_x = self._radius * cos(time)
		circle_y = self._radius * sin(time)
